...
void Game::Splat(int k){
	...
	if(k!=0||visions==0){
		...
		ALuint snd = SoundFX::inst()->getFreeSource(gSampleSet[headwhacksound]);
		//alSourcei(gSourceID[src_headwhacksound], AL_BUFFER, gSampleSet[headwhacksound]);
...
void 	Game::Tick(){
	...
	if(mainmenu){
		...
		if((Button()&&mouseoverbutton==1&&!gameinprogress&&!oldbutton)||!mainmenuness){
			...
			{
				ALuint env = SoundFX::inst()->getEnviroSource(gSampleSet[rainsound]);
				alSourcef(env, AL_MIN_GAIN, .3);
				alSourcePlay(env);
			}

			if(environment!=rainy_environment)
			{
				ALuint env = SoundFX::inst()->getEnviroSource();
				alSourcePause(env);
			}
			...
			SoundFX::inst()->playFX(gSampleSet[soulinsound], gLoc, 1.0);
		...
		if((Button()&&mouseoverbutton==1&&gameinprogress&&!oldbutton)||!mainmenuness){
			...
			//if(!visions){
				...
				ALuint enviro = SoundFX::inst()->getEnviroSource();
			...
			if(visions)
			{
				ALuint vis = SoundFX::inst()->getVisionSource(gSampleSet[visionsound]);
				alSourcefv(vis, AL_POSITION, gLoc);
				alSourcePlay(vis);
			}
		...
		if(Button()&&mouseoverbutton==2&&!gameinprogress&&!oldbutton){
			...
			alSourcePlay(SoundFX::inst()->getFreeSource(gSampleSet[losesound], 1.0));
	...
	if(!mainmenu){
		...
		if(timeremaining<=0){
			...
			//alSourcePlay(SoundFX::inst()->getFreeSource(gSampleSet[souloutsound], 1.0));
			float gLoc[3];
			getAuralCameraPosition(gLoc);
			SoundFX::inst()->playFX(gSampleSet[souloutsound], gLoc, 1.0f);
			...
			if(mission>=nummissions){
				...
				alSourcePause(SoundFX::inst()->getEnviroSource());
			...
			if(!mainmenu){
				...
				if(environment==rainy_environment)
				{
					...
					//alSourcePlay(gSourceID[src_rainsound]);
				}

				if(environment!=rainy_environment)
				{
					alSourcePause(SoundFX::inst()->getEnviroSource());
					//alSourcePause(gSourceID[src_rainsound]);
				}
		...
		if(IsKeyDown(theKeyMap, psychickey)&&!oldvisionkey){
			...
			if(visions==0){
				...
				alSourcef(SoundFX::inst()->getMusicSource(), AL_PITCH, (ALfloat)(1));
			}
			...
			if(visions==1){
				...
				alSourcef(SoundFX::inst()->getMusicSource(), AL_PITCH, (ALfloat)(.5));
...
//		XYZ pointrotated;
		...
		if(spawndelay<0&&numpeople<max_people){
			...
			if(person[numpeople].type!=civiliantype&&blockspawnx==person[1].whichblockx&&blockspawny==person[0].whichblocky){
		...
		if(spawndelay<0&&numpeople>=max_people){
			...
			if(cycle<max_people&&cyclenum<max_people){
				...
				if(person[cycle].type!=civiliantype&&blockspawnx==person[0].whichblockx&&blockspawny==person[1].whichblocky){
									 ...
									 if(person[i].killtargetvisible){
									 	...
									 	if((findDistancefast(person[i].playercoords, person[person[i].killtarget].playercoords)<=tooclose&&person[person[i].killtarget].skeleton.free==0)||(tooclose>200&&findDistancefast(person[i].playercoords, person[person[i].killtarget].playercoords)<=200)||(tooclose<=200&&findDistancefast(person[i].playercoords, person[person[i].killtarget].playercoords)<tooclose)){
											...
											if(person[i].whichgun==knife&&person[person[i].killtarget].health==100){
												...
												{
													//alSourcei(gSourceID[src_knifeslashsound], AL_BUFFER, gSampleSet[knifeslashsound]);
													SoundFX::inst()->playFX(gSampleSet[knifeslashsound], gLoc);
		...
		if(Button()&&person[0].whichgun==grenade&&person[0].ammo>0&&person[0].reloading<=0&&person[0].attackframe<0&&person[0].targetanimation!=crouchanim){
			...
			if(person[0].grenphase==0){
				...
				ALuint snd = SoundFX::inst()->getFreeSource(gSampleSet[pinpullsound]);
				//alSourcei(gSourceID[src_pinpullsound], AL_BUFFER, gSampleSet[pinpullsound]);
				alSourcefv(snd, AL_POSITION, gLoc);
				alSourcePlay(snd);
		...
		if(person[0].targetanimation==crouchanim){
			...
			if(person[0].grenphase==1){
				...
				ALuint snd = SoundFX::inst()->getFreeSource(gSampleSet[pinreplacesound]);
				//alSourcei(gSourceID[src_pinreplacesound], AL_BUFFER, gSampleSet[pinreplacesound]);
				alSourcefv(snd, AL_POSITION, gLoc);
		...
		if(Button()&&!oldbutton&&(person[0].aiming==0||person[0].whichgun==grenade||person[0].whichgun==nogun||person[0].whichgun==knife)&&person[0].currentanimation==crouchanim){
			...
			for(int i=0;i<max_people;i++){
				...
				if(!switched&&person[i].skeleton.free==1&&findDistancefast(person[0].playercoords,person[i].averageloc)<200){
					...
					ALuint snd = SoundFX::inst()->getFreeSource(gSampleSet[clicksound]);
					//alSourcei(gSourceID[src_clicksound], AL_BUFFER, gSampleSet[clicksound]);
					alSourcefv(snd, AL_POSITION, gLoc);
					alSourcePlay(snd);
			...
			if(person[person[0].killtarget].skeleton.free<1&&person[0].killtarget!=0&&(person[0].aiming<1||person[0].whichgun==nogun||person[0].whichgun==knife||person[0].targetanimation==joganim)){
...

				{
					...
					if(person[0].whichgun==knife){
						...
						ALuint snd = SoundFX::inst()->getFreeSource(gSampleSet[knifeslashsound]);
						//alSourcei(gSourceID[src_knifeslashsound], AL_BUFFER, gSampleSet[knifeslashsound]);
						alSourcefv(snd, AL_POSITION, gLoc);
						alSourcePlay(snd);
			...
			if(firing){
				...
				if(person[j].aiming>=1&&person[j].recoil<=0){
					...
					if(person[j].whichgun==sniperrifle){
						...
						SoundFX::inst()->playFX(gSampleSet[riflesound], gLoc);

						//alSourcei(gSourceID[src_riflesound], AL_BUFFER, gSampleSet[riflesound]);
					...
					if(person[j].whichgun==shotgun){
						...
						SoundFX::inst()->playFX(gSampleSet[shotgunsound], gLoc);
						//alSourcei(gSourceID[src_shotgunsound], AL_BUFFER, gSampleSet[shotgunsound]);
					...
					if(person[j].whichgun==handgun1){
						...
						SoundFX::inst()->playFX(gSampleSet[pistol1sound], gLoc);
						//alSourcei(gSourceID[src_pistol1sound], AL_BUFFER, gSampleSet[pistol1sound]);
					...
					if(person[j].whichgun==handgun2){
						...
						SoundFX::inst()->playFX(gSampleSet[pistol2sound], gLoc);
						//alSourcei(gSourceID[src_pistol2sound], AL_BUFFER, gSampleSet[pistol2sound]);
					...
					if(person[j].whichgun==assaultrifle){
						...
						SoundFX::inst()->playFX(gSampleSet[machinegunsound], gLoc);
						//alSourcei(gSourceID[src_machinegunsound], AL_BUFFER, gSampleSet[machinegunsound]);
					...
					for(int m=0;m<bulletstrength;m++){
						...
						if(m==0){
							...
							if(j==0&&slomo==2){
								...
								if(whichhit==-1)
									alSourcePlay(SoundFX::inst()->getFreeSource(gSampleSet[disguisekillsound], 1));
									//alSourcePlay(gSourceID[src_disguisekillsound]);
						...
						if(whichhit!=-1&&whichhit!=firstpass){
							...
							if(hitstruct.joint1->modelnum==headmodel){
								...
								if(!thirdperson)//alSourcef(gSourceID[src_headshotsound], AL_MIN_GAIN, 1);
									SoundFX::inst()->playFX(gSampleSet[headshotsound], gLoc, 1.0);

								if(thirdperson)//alSourcef(gSourceID[src_headshotsound], AL_MIN_GAIN, .1);
									SoundFX::inst()->playFX(gSampleSet[headshotsound], gLoc, 0.1);

								//alSourcei(gSourceID[src_headshotsound], AL_BUFFER, gSampleSet[headshotsound]);
								//alSourcefv(gSourceID[src_headshotsound], AL_POSITION, gLoc);

								//alSourcePlay(gSourceID[src_headshotsound]);
						...
						if(oldend==finalwallhit){
							...
							SoundFX::inst()->playFX(gSampleSet[wallhitsound], gLoc, 0.0, 0.6, true);

							//alSourcei(gSourceID[src_wallhitsound], AL_BUFFER, gSampleSet[wallhitsound]);
							//alSourcefv(gSourceID[src_wallhitsound], AL_POSITION, gLoc);
							//alSourcePlay(gSourceID[src_wallhitsound]);
						...
						*a+=aim*1;
						...
						if(nearest.x){
							...
							if(findDistancefast(nearest,camera.position)<10&&(thirdperson==2||j!=0)){
								...
								SoundFX::inst()->playFX(gSampleSet[nearbulletsound], gLoc);
								/*
								alSourcei(gSourceID[src_nearbulletsound], AL_BUFFER, gSampleSet[nearbulletsound]);
								alSourcefv(gSourceID[src_nearbulletsound], AL_POSITION, gLoc);
								alSourcePlay(gSourceID[src_nearbulletsound]);
								*/
...
//							alSourcei(gSourceID[src_bouncesound], AL_BUFFER, gSampleSet[bouncesound+whichsound]);
//							alSourcefv(gSourceID[src_bouncesound], AL_POSITION, gLoc);
							if(sprites.size[i]<=1)//alSourcePlay(gSourceID[src_bouncesound+whichsound]);			
								SoundFX::inst()->playFX(gSampleSet[bouncesound + whichsound], gLoc);
							...
							if((j!=0||sprites.brightness[i]<.9)&&person[j].existing){
								...
								if(hitstruct.collision){
									...
									if((hitstruct.joint1->label==head||hitstruct.joint2->label==head)&&person[j].type!=zombietype){
										...
										if(sprites.size[i]<=1)//alSourcePlay(gSourceID[src_headwhacksound]);
											SoundFX::inst()->playFX(gSampleSet[headwhacksound], gLoc);
				...
				if(sprites.type[i]==grenadesprite){
					...
					if(sprites.brightness[i]<=0||(impact&&sprites.size[i]>1)){
						...
						/*
						alSourcei(gSourceID[src_explosionsound], AL_BUFFER, gSampleSet[explosionsound]);
						alSourcefv(gSourceID[src_explosionsound], AL_POSITION, gLoc);
						alSourcePlay(gSourceID[src_explosionsound]);
						*/
						SoundFX::inst()->playFX(gSampleSet[explosionsound], gLoc);
		...
		for(int i=0;i<numpeople;i++){
			...
			if(person[i].oldhealth>0&&person[i].health<=0){
				...
				if(i==1)//alSourcePlay(gSourceID[src_losesound]);	
				{
					float gLoc[3];
					getAuralCameraPosition(gLoc);
					SoundFX::inst()->playFX(gSampleSet[losesound], gLoc, 1.0f);
				}
				...
				if(person[i].type==civiliantype)
				{
					...
					float gLoc[3];
					getAuralCameraPosition(gLoc);
					SoundFX::inst()->playFX(gSampleSet[disguisekillsound], gLoc, 1.0f);
				...
				if(person[i].type==eviltype)
				{
					...
					float gLoc[3];
					getAuralCameraPosition(gLoc);
					SoundFX::inst()->playFX(gSampleSet[soulinsound], gLoc, 1.0f);
...
//		ALint tempInt;
		...
		// hjk - put music in right spot
		ALuint music = SoundFX::inst()->getMusicSource();
		alSourcefv(music, AL_POSITION, gLoc);

		// hjk - put rain in right spot
		ALuint enviro = SoundFX::inst()->getEnviroSource();
		alSourcefv(enviro, AL_POSITION, gLoc);
...
void Game::getAuralCameraPosition(float *gLoc)
{
	gLoc[0]=camera.position.x/soundscalefactor;
	gLoc[1]=camera.position.y/soundscalefactor;
	gLoc[2]=camera.position.z/soundscalefactor;
