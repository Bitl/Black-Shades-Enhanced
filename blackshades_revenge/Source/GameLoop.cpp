...

	{
		...
		case 'b':
			...
			if(debug)
			{
				...
				SoundFX::inst()->playFX(gSampleSet[soulinsound], NULL, 1.0);
			...
			// Slow motion sound
			alSourcef(SoundFX::inst()->getMusicSource(), AL_PITCH, slomo?((ALfloat)(.5)):((ALfloat)(1.0)));
		...
		case 'f':
			...
			SoundFX::inst()->playFX(gSampleSet[souloutsound], NULL, 1.0f);
...

{
...

	{
		...
		if ( !mainmenu && IsKeyDown( theKeyMap, MAC_ESCAPE_KEY )){
			...
			// hjk - Added because the soulout was blowing out my speaker cones.
			gLoc[2] = 3.0;
			SoundFX::inst()->playFX(gSampleSet[souloutsound],gLoc, 0.1, 0.1);
