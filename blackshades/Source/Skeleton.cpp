...
Skeleton::Skeleton() : m_bodyland(false), m_headland(false)
{
		...
		for(int i=0; i<num_joints; i++){
			...
			if(joints[i].position.y<=groundlevel+offset||whichtri!=-1){
				...
				if(joints[i].label==head&&(findLengthfast(joints[i].velocity)>2||findLengthfast(avgvelocity)>2)){
					...
					alGetSourceiv(gSourceID[src_headlandsound], AL_SOURCE_STATE, &tempint);
#else
					alGetSourcei(gSourceID[src_headlandsound], AL_SOURCE_STATE, &tempint);
#endif

					if (tempint != AL_PLAYING){
						alSourcei(gSourceID[src_headlandsound], AL_BUFFER, gSampleSet[headlandsound]);
						alSourcef(gSourceID[src_headlandsound], AL_MIN_GAIN, ALfloat(findLengthfast(avgvelocity)*2/findDistancefast(joints[i].position,camera.position)*soundscalefactor*2));
						alSourcef(gSourceID[src_headlandsound], AL_MAX_GAIN, ALfloat(findLengthfast(avgvelocity)*2/findDistancefast(joints[i].position,camera.position)*soundscalefactor*2));						
						alSourcefv(gSourceID[src_headlandsound], AL_POSITION, gLoc);
						alSourcePlay(gSourceID[src_headlandsound]);}
*/

					// I don't think above is needed... We'll see?
					
					if (m_headland)
					{
						m_headland = true;
						SoundFX::inst()->playFX(gSampleSet[headlandsound], gLoc,
												(findLengthfast(avgvelocity)*2/findDistancefast(joints[i].position,camera.position)*soundscalefactor*2),
												(findLengthfast(avgvelocity)*2/findDistancefast(joints[i].position,camera.position)*soundscalefactor*2));
					}						

				}
				else if(joints[i].label==head)
				{
					m_headland = false;
				}
...
/*
#ifdef DEBIAN_NEEDS_TO_UPDATE_THEIR_OPENAL
					alGetSourceiv(gSourceID[src_bodylandsound], AL_SOURCE_STATE, &tempint);
#else
					alGetSourcei(gSourceID[src_bodylandsound], AL_SOURCE_STATE, &tempint);
#endif					
					if (tempint != AL_PLAYING){
						alSourcei(gSourceID[src_bodylandsound], AL_BUFFER, gSampleSet[bodylandsound]);
						alSourcef(gSourceID[src_bodylandsound], AL_MIN_GAIN, ALfloat(findLengthfast(joints[i].velocity)*1/findDistancefast(joints[i].position,camera.position)*soundscalefactor*2));
						alSourcef(gSourceID[src_bodylandsound], AL_MAX_GAIN, ALfloat(findLengthfast(joints[i].velocity)*1/findDistancefast(joints[i].position,camera.position)*soundscalefactor*2));
						alSourcefv(gSourceID[src_bodylandsound], AL_POSITION, gLoc);
						alSourcePlay(gSourceID[src_bodylandsound]);}
*/
					...
					{
						m_bodyland = true;
						SoundFX::inst()->playFX(gSampleSet[bodylandsound], gLoc,
												(findLengthfast(joints[i].velocity)*1/findDistancefast(joints[i].position,camera.position)*soundscalefactor*2),
												(findLengthfast(joints[i].velocity)*1/findDistancefast(joints[i].position,camera.position)*soundscalefactor*2));
					}
											
				}
				else if(joints[i].label==abdomen)
				{
					m_bodyland = false;
				}
