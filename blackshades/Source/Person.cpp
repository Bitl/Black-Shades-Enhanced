...
void 	Person::DoAnimations(int who){	
	...
	if(target>1&&!skeleton.free){
		...
		if(who==0&&slomo==0&&(targetanimation==joganim||targetanimation==walkanim)&&(targetframe==0||targetframe==8)&&visions==0&&(onground||abs(velocity.y)<1)){
			...
			SoundFX::inst()->playFX(gSampleSet[whichsound], gLoc);

			//alSourcei(gSourceID[src_footstepsound], AL_BUFFER, gSampleSet[whichsound]);
...
//			int whichsound;
		...
		if(targetanimation==diveanim&&currentanimation==diveanim&&targetframe==0){
			...
			//alSourcePlay(gSourceID[src_bodywhacksound]);
		}
		...
		if(attackframe>-1&&whichgun==grenade&&ammo>0){
			...
			if(attacktarget>1){
				...
				if(attackframe==animation[grenadethrowanim].numframes-1&&ammo>0){
					...
					SoundFX::inst()->playFX(gSampleSet[grenadethrowsound], gLoc);
					//alSourcei(gSourceID[src_grenadethrowsound], AL_BUFFER, gSampleSet[grenadethrowsound]);
					//alSourcefv(gSourceID[src_grenadethrowsound], AL_POSITION, gLoc);
					//alSourcePlay(gSourceID[src_grenadethrowsound]);
				}
			}
		}
...
/*
					...
					alGetSourceiv(gSourceID[src_reloadsound], AL_SOURCE_STATE, &tempint);
#else					
					alGetSourcei(gSourceID[src_reloadsound], AL_SOURCE_STATE, &tempint);
#endif

					if (tempint != AL_PLAYING){
						alSourcei(gSourceID[src_reloadsound], AL_BUFFER, gSampleSet[reloadsound]);
						alSourcefv(gSourceID[src_reloadsound], AL_POSITION, gLoc);
						alSourcePlay(gSourceID[src_reloadsound]);
					}

  */
					// I don't think aboveis needed... We'll see.
					SoundFX::inst()->playFX(gSampleSet[reloadsound], gLoc);
				}
				reloading=3;
				aiming=0;
...
void 	Person::DoAnimationslite(int who){
	...
	if(target>1&&!skeleton.free){
		...
		if(who==0&&(targetanimation==joganim||targetanimation==walkanim)&&(targetframe==0||targetframe==8)&&visions==0&&(onground||abs(velocity.y)<1)){
			...
			//alSourcePlay(gSourceID[src_footstepsound]);
		}
		if(targetanimation==zombieeatanim&&(targetframe==3)){
//			int whichsound;
			float gLoc[3];
