...
FILE *Files::OpenFile(Str255 Name)
{
	char filename1[MAX_PATH];
...
#ifdef __APPLE_
	//alutLoadWAVFile(filename1, wave, &format1, &size1, &bits1, &freq1);
	alutLoadWAVFile(filename1,(int *)&format1,wave,&size1,&freq1);
#endif
#ifdef WIN32
	ALboolean loop;
 	alutLoadWAVFile(filename1,(int *)&format1,wave,&size1,&freq1,&loop);
#endif
#ifdef __linux__
	ALboolean loop;
 	alutLoadWAVFile((ALbyte *)filename1,(int *)&format1,wave,&size1,&freq1,&loop);
#endif
